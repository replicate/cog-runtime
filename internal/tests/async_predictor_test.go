...
func TestAsyncPredictorSucceeded(t *testing.T) {
	...
	assertResponse(t, barR[2], server.PredictionProcessing, nil, barLogs)
	barLogs += "completed async prediction\n"
	assertResponse(t, barR[3], server.PredictionSucceeded, "*bar*", barLogs)
	...
	assertResponse(t, bazR[3], server.PredictionProcessing, nil, bazLogs)
	assertResponse(t, bazR[4], server.PredictionProcessing, nil, bazLogs)
	bazLogs += "completed async prediction\n"
	assertResponse(t, bazR[5], server.PredictionSucceeded, "*baz*", bazLogs)
